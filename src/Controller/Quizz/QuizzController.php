...
class QuizzController extends AbstractController
{
    ...
    public function edit(Request $request, Quizz $quizz, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
    {
        $form = $this->createForm(QuizzType::class, $quizz);
        $form->handleRequest($request);
            ...
            if($illustrationFile){
                ...
                try {
                    $illustrationFile->move(
                        $this->getParameter('quizzs_directory'),
                        $newFilename
                    );
                } catch (FileException $e) {
                    // ... handle exception if something happens during file upload
                }
                $image = new QuizzIllustrations();
                $image->setFile($newFilename);
                $quizz->addQuizzIllustration($image);
            }
